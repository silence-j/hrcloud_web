...
import { reMarketUser } from 'static/api/personnel-information/base/base'
...
export default {
  ...
  methods: {
    goBack () {
      this.$router.push('/market-verify/entry/owner')
    },
    goReview: debounce(function (e) {
      ...
      if (this.vaildate) {
        ...
        reMarketUser(addParams).then(data => {
          if (this.employeeId) {
            saveSalaryStructure(Object.assign({}, salaryParams, { userId: this.employeeId, deleteTag: 5 })).then(result => {})
          }
