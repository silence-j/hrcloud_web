<template>
  <div class="container">
    <el-tabs type="border-card" @tab-click="tabClick" class="mt-20">
      <el-tab-pane>
        ...
        <el-table
          ...
          <el-table-column
           ...
           <div v-html="scope.row.updateContect"></div>
            <!--   <div class="moveBtn">
                <el-tooltip placement="top" effect="light">
                  <div slot="content" class="text-content"><div v-html="scope.row.updateContect"></div></div>
                  <div v-html="scope.row.updateContect"></div>
                </el-tooltip>
              </div> -->
            </template>
      ...
      <el-tab-pane v-if="getCurrentRoleGroups.indexOf('contractMarketUpdate')!==-1">
        <span slot="label">待审核</span>
        <el-table :data="verifywaitDatas" border>
          <el-table-column
            label="序号"
            type="index"
            width="60">
      ...
      <el-tab-pane v-if="getCurrentRoleGroups.indexOf('contractMarketUpdate')!==-1">
        ...
        <el-table :data="verifyFinishDatas" border>
          <el-table-column
            label="序号"
            type="index"
            width="60">
...
export default {
  ...
  methods: {
    ...
    /** 待审核**/
    _getwaitTaskList () {
      getTaskList(Object.assign(this.contractWaitParams, { groupId: 'contractMarketUpdate' })).then(data => {
        if (data.result) {
          if (data.result.records) {
