      ...
      <div class="tab-wrap clearfix">
        <ul class="edit-tab clearfix">
          <li @click="emptyClearMsg" v-show="permission.indexOf('合同变更我发起的') !=-1">
             <router-link :to="'/market-verify/contract/change'">合同变更</router-link>
          </li>
          <li @click="emptyClearMsg" v-show="permission.indexOf('合同解除我发起的') !=-1">
            <router-link :to="'/market-verify/contract/move'">合同解除</router-link>
          </li>
          <li @click="emptyClearMsg" v-show="permission.indexOf('合同作废我发起的') !=-1">
            <router-link :to="'/market-verify/contract/cancel'">作废</router-link>
          </li>
        </ul>
      </div>
    </el-col>
...
export default {
  ...
  computed: {
    ...
    /* ...mapGetters(['getPersonnalBaseStatusSelect']), */
    permission () {
      return getExmine().marketVerifyContract
    }
  },
  methods: {
    setSerchList (boolean) {
      this.$store.dispatch('setMarketVerifyContractSearch', this.contractForm)
      this.$store.commit('SET_MARKETVERIFYCONTRACT_CLICKED', boolean)
    },
    searchContractList () {
