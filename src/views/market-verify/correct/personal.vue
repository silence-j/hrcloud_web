  ...
  methods: {
    ...
    tabClick (tab, event) {
      if (tab.index === '0') {
        ...
        // this._getOwnerTaskList(1)
      } else if (tab.index === '1') {
        this.tabTitle = '已审核'
        // this._getwaitTaskList(1)
    ...
    /** 已审核**/
    _getwaitTaskList () {
      getTaskFinshed(Object.assign(this.taskFinshedParams)).then(data => {
        if (data.result) {
          if (data.result.records) {
      ...
      this.$refs.reasonForm.validate(valid => {
        if (valid) {
          ...
          let params = {
            ...
            var: {
              keys: 'correctMarketHrPass,correctMarketHrBackReason',
