  ...
  <div class="container">
    ...
    class="mt-20"
     @tab-click="tabClick">
      ...
      <el-tab-pane v-if="getCurrentRoleGroups.indexOf('dutyMarketDepose')!==-1">
...
export default {
  ...
  computed: {
    ...mapGetters(['getMarketJobSearchArr', 'getMarketVerifyJobClick', 'getCurrentRoleGroups']),
    contractOwnerParams () {
      return {
        actType: 'dutyMarketDepose',
        userId: getToken('loginId').toString(),
        keyword: this.getMarketJobSearchArr.keyword,
        startTime: this.getMarketJobSearchArr.startTime ? timeZeroToString(this.getMarketJobSearchArr.startTime) : '',
        endTime: this.getMarketJobSearchArr.endTime ? timeZeroToString(this.getMarketJobSearchArr.endTime) : '',
  ...
  methods: {
    ...
    _getwaitTaskList () {
      /** 待审核**/
      getTaskList(Object.assign(this.contractWaitParams, { groupId: 'dutyMarketDepose' })).then(data => {
        if (data.result) {
          if (data.result.records) {
  ...
  watch: {
    getMarketVerifyJobClick (val) {
      ...
      this.$store.commit('SET_MARKETVERIFYJOB_CLICKED', false)
    }
  },
  created () {
    this._getOwnerTaskList()
