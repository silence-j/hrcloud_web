...
<div class="container">
    ...
    class="mt-20"
     @tab-click="tabClick">
      ...
      <el-dialog title="同意" :visible.sync="agreeVisible" width="700px">
        <el-form :model="agreeForm" label-width="115px" class="dialog-form">
          ...
          <el-form-item label="工资标准：" >
            <span>{{receiveDetailDatas.newSalaryForm ? receiveDetailDatas.newSalaryForm :receiveDetailDatas.oldSalaryForm}}</span>
          </el-form-item>
          ...
          <el-form-item label="新  部  门：">
            {{receiveDetailDatas.toDeptName}}
          </el-form-item>
          <el-form-item label="新  岗  位：">
              {{receiveDetailDatas.newPostName}}
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    agreeSave () {
      ...
      let transferParams = {
        ...
        status: 1
        // salaryMonthInfo: this.receiveDetailDatas.salaryMonthInfo
      }
    ...
    _getDetail (row, tab) {
      getReceiveDetail2(Number(row.changeId)).then(data => {
        if (data.result) {
          ...
          console.log(this.receiveDetailDatas)
