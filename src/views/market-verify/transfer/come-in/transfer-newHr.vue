...
<div class="container" @click="orgDatas.showTree = false">
    ...
    <el-dialog title="接收" :visible.sync="agreeVisible" v-draggable width="900px">
      <el-form
        :model="agreeForm"
        ref="agreeForm"
        :rules="agressRules"
        ...
        <el-form-item label="新  部  门："  required>
          <search-dept-tree
          :orgDatas="orgDatas"
    ...
    <el-dialog title="不接收原因" :visible.sync="UnagreeVisible" v-draggable width="700px">
      <el-form
        :model="UnagreeForm"
        ref="unagreeForm"
        :rules="unagreeRules"
...
import { getTaskComplete, saveTransferMessage } from 'static/api/exmine/exmine'
...
export default {
  ...
  methods: {
    ...
    AgreeMsg (row) {
      ...
      this.agreeForm.newPostName = row.postName
      ...
      this._getAceptDetail()
      this.agreeForm.newPostName = ''
    ...
    _getAceptDetail () {
      getReceiveDetail2(Number(this.agreeForm.changeId)).then(data => {
        if (data.result) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          saveSalaryStructure(regularSalaryParams).then(result => {
            saveTransferMessage(transferParams).then(data => {
