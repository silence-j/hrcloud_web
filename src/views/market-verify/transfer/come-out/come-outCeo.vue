<template>
<div class="container" @click="showTreeTag=false;showTreeTag1=false">
  <el-tabs
    type="border-card"
    class="mt-20"
     @tab-click="tabClick">
            ...
            <template slot-scope="scope">
              ...
              <div v-else>
                 <el-tooltip placement="top" effect="light">
                   <div slot="content" style="width: 130px;height: 55px;overflow: auto;" v-if="scope.row.historicVariableInstance">{{scope.row.historicVariableInstance.value}}</div>
                  <span>不同意</span>
                </el-tooltip>
              </div>
            </template>
      ...
      <el-dialog title="同意" :visible.sync="agreeVisible" v-draggable width="700px">
        <el-form :model="agreeForm" label-width="115px" class="dialog-form">
          ...
          <el-form-item label="原单位：">
            <span>{{receiveDetailDatas.fromCompanyName}}</span>
          </el-form-item>
          <el-form-item label="原部门：">
            <span>{{receiveDetailDatas.fromDeptName}}</span>
          </el-form-item>
          <el-form-item label="原岗位：">
            <span>{{receiveDetailDatas.oldPostName}}</span>
          </el-form-item>
          <!--<el-form-item label="新部门：">
              <span>{{receiveDetailDatas.toDeptName}}</span>
            </el-form-item>-->
        </el-form>
      ...
      <el-dialog title="不同意原因" :visible.sync="UnagreeVisible" v-draggable width="700px">
        <el-form
          ref="unagreeForm"
          :rules="unagreeRules"
          :model="UnagreeForm"
...
export default {
  ...
  methods: {
    ...
    showDeptTree1 () {
      this.showTreeTag1 = true
    },
    getCompanyId (val) {
      this.moveForm.orgName = val.split('-')[0]
    },
    getDeptId (val) {
      this.movePostForm.toDeptName = val.split('-')[0]
    },
    getPostId (val) {
      this.agreeForm.newDeptId = val.split('-')[0]
      this.agreeForm.newDeptName = val.split('-')[1]
    },
    /** 同意**/
    AgreeMsg (row) {
