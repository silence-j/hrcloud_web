          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <div v-else>
                ...
                <div v-else>
                  <span :title="scope.row.reason">不接收</span>
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="operation">
                ...
                <span v-if="scope.row.task&&scope.row.task.name=='新部门人事审核'" @click="revoke(scope.row)">撤销</span>
          ...
          <el-form label-width="115px" class="dialog-form">
            ...
            <template v-if="detailType === 1">
              <el-form-item label="新部门：">
                <span>{{receiveDetailDatas.toCompanyName}}-{{receiveDetailDatas.toCompanyName}}</span>
              </el-form-item>
...
export default {
  ...
  methods: {
    ...
    transferMsg (row) {
      ...
      this.$store.dispatch('setSingleTreeCheckedName', row.newDeptName)
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let transferParams = {
            ...
            toCompanyId: this.moveForm.orgId,
