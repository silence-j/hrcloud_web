<template>
  <div @click="mutilTreeData.showTree = false">
    <el-button @click="ExportMsg" style="position: absolute;top: 0;" type="primary">导出</el-button>
    <el-form :model="searchForm" class="mt-20">
      ...
      <el-row :gutter="20" type="flex">
        <el-col :span="4">
          <el-date-picker
            v-model="searchForm.checkDateStart"
            type="date"
            clearable
        ...
        <el-col :span="4">
          <el-date-picker
            v-model="searchForm.checkDateEnd"
            type="date"
            clearable
            :picker-options="checkPickerOptions"
    ...
    <div class="mt-20">
      <el-table :data="list">
        ...
        <el-table-column label="审核人" prop="updator"></el-table-column>
    ...
    <form style="display:none" name="form" method="POST" ref="submitExcel" id="submitExcel" :action="hostBaseUrl+'/user/userChangeData/makeExport'">
      ...
      <p>auditBeginDate: <input type="text" id="auditBeginDate" name="auditBeginDate" value="" /></p>
      <p>auditEndDate: <input type="text" id="auditEndDate" name="auditEndDate" value="" /></p>
...
// import { fieldLists } from 'assets/js/fields'
...
export default {
  ...
  computed: {
    pickerOptions () {
      return {
        disabledDate: (time) => {
          return new Date(time).getTime() < new Date(this.searchForm.applyDateStart).getTime()
        }
      }
    },
    checkPickerOptions () {
      return {
        disabledDate: (time) => {
          return new Date(time).getTime() < new Date(this.searchForm.checkDateStart).getTime()
        }
      }
    },
    paramsList () {
      return {
        ...
        beginDate: this.searchForm.applyDateStart ? timeZeroToString(this.searchForm.applyDateStart) : '',
        endDate: this.searchForm.applyDateEnd ? timeZeroToString(this.searchForm.applyDateEnd) : '',
        auditEndDate: this.searchForm.checkDateEnd ? timeZeroToString(this.searchForm.checkDateEnd) : '',
        changeNames: this.searchForm.field ? this.searchForm.field.join(',') : null,
        auditBeginDate: this.searchForm.checkDateStart ? timeZeroToString(this.searchForm.checkDateStart) : '',
  ...
  methods: {
    ExportMsg () {
      ...
      document.getElementById('beginDate').setAttribute('value', this.searchForm.applyDateStart ? timeZeroToString(this.searchForm.applyDateStart) : '')
      document.getElementById('endDate').setAttribute('value', this.searchForm.applyDateEnd ? timeZeroToString(this.searchForm.applyDateEnd) : '')
      document.getElementById('auditBeginDate').setAttribute('value', this.searchForm.checkDateStart ? timeZeroToString(this.searchForm.checkDateStart) : '')
      document.getElementById('auditEndDate').setAttribute('value', this.searchForm.checkDateEnd ? timeZeroToString(this.searchForm.checkDateEnd) : '')
