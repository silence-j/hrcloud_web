<template>
  <div class="mt-20 table-setting-warp" @click="orgDatas.showTree = false">
    ...
    <el-table
      :data="allFundList" @sort-change="sortChange"
      border>
      <el-table-column
        label="序号"
        type="index"
        ...
        <template slot-scope="scope">
          <div class="operation">
            <div v-if="scope.row.canOperate === 1">
              <span v-show="permission.indexOf('办理') !=-1 && (scope.row.status === 0 || scope.row.status === 1 )" @click="handleMsg(scope.row)">办理</span>
              <span v-show="permission.indexOf('变更') !=-1 && scope.row.status === 2" @click="changeMsg(scope.row)">变更</span>
              <span v-show="permission.indexOf('停缴') !=-1 && scope.row.status === 2" @click="stopMsg(scope.row)">停缴</span>
              <span v-show="permission.indexOf('不缴纳') !=-1 && scope.row.status === 1" @click="NotTopayMsg(scope.row)">不缴纳</span>
  ...
  computed: {
    ...
    listParams () {
      return {
        ...
        status: this.getFundSearchArr.status,
      ...
      getFundList(this.listParams).then(data => {
        if (data.result) {
          if (data.result.records) {
            this.allFundList = data.result.records
          }
