        ...
        <div v-for="item in fieldList" :key="item.id">
          ...
          <template v-else-if="item.prop == 'status'">
            <el-table-column
              ...
              width="120px" :sortable="item.orderFlag === 1">
              <template slot-scope="scope">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="operation">
              ...
              <span v-show="scope.row.status !== 3 && scope.row.status !== 15&&permission.indexOf('编辑')!=-1" @click="acceptEdit(scope.row)">编辑</span>
              <span v-show="scope.row.status == 0&&permission.indexOf('面试评估')!=-1" @click="classifyMsg(scope.row)">面试评估</span>
              <span v-show="permission.indexOf('面试评估')!=-1 && (scope.row.status == 1 || scope.row.status == 4) " @click="classifyMsg(scope.row)">重新评估</span>
              <span v-show="permission.indexOf('审批进度')!=-1&&scope.row.isMarketing !== 1" @click="progressData(scope.row)">审核进度</span>
            </div>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  computed: {
    ...mapGetters(['getAcceptTalentSearchArr', 'getAcceptTalentClick', 'getDeepSearchOptions', 'getChooseCheck']),
    permission () {
