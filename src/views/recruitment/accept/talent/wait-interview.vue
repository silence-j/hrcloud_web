<template>
  <div class="all-box">
      ...
      <el-table
        ...
        <div v-for="item in fieldList" :key="item.id">
          <template v-if="item.prop == 'name'">
            <el-table-column
              label="姓名" prop="name" :sortable="item.orderFlag === 1"
              min-width="120px">
          ...
          <template v-else-if="item.prop == 'status'">
            <el-table-column label="人才分类" prop="status"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="operation">
              <span @click="acceptCheck(scope.row)" v-show="permission.indexOf('查看')!=-1">查看</span>
              <span v-show="permission.indexOf('编辑')!==-1&&scope.row.status !== 3 && scope.row.status !== 15" @click="acceptEdit(scope.row)">编辑</span>
              <span v-show="scope.row.status==0 && permission.indexOf('面试评估')!=-1 " @click="classifyMsg(scope.row)">评估</span>
              <span v-show="permission.indexOf('面试评估')!=-1&&(scope.row.status == 1 || scope.row.status == 4)" @click="classifyMsg(scope.row)">重新评估</span>
              <span @click="deleteData(scope.row)" v-show="permission.indexOf('删除')!=-1">删除</span>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!--<el-dialog-->
      ...
      <!--:visible.sync="isDel"-->
      <!--width="300px">-->
      <!--<span class="content">删除后无法恢复，确定要删除？</span>-->
...
export default {
  ...
  computed: {
    ...mapGetters(['getAcceptTalentSearchArr', 'getAcceptTalentClick', 'getIsDel', 'getDeepSearchOptions', 'getChooseCheck']),
    permission () {
  ...
  watch: {
    getAcceptTalentClick (val) {
      ...
      this.setAcceptClick(false) // 重置搜索
    },
    getIsDel (val) {
      if (val) {
        this.pageMsg.currentPage = 1
