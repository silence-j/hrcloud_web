...
<div class="market-report-mainDetail_box">
  <el-row>
    <el-button
     size="mini"
     type="primary"
     @click="$router.push({name: 'ReportMarketMain'})">返回</el-button>
  ...
  <el-row class="mt-20">
       <div class="table-div_box">
        ...
        <div class="table-body">
          <ul class="clearfix" v-for="(item,index) in mainReportDatas" :key="item.id">
            ...
            <li style="width:200px;" class="click-li">
              <el-tooltip v-if="item.name.length > 6" class="item" :content="item.name" effect="light"  placement="top-start">
                <span @click="skipDetail(item)">{{item.name}}</span>
              </el-tooltip>
              <span v-else @click="skipDetail(item)">{{item.name}}</span>
            </li>
   ...
   ref="submitExcel" id="submitExcel" :action="hostBaseUrl+'/report/monthlyReportForms/queryMonthlyMain/hrBelong/export'">
      ...
      <p>type: <input type="text" id="type" name="type" value="1" /></p>
...
export default {
  ...
  methods: {
    skipDetail (item) {
      ...
      this.$router.push({ name: 'ReportMarketDeptDetail',
        query: { groupIds: JSON.stringify(item.groupIds), companyName: item.name, companyId: item.id },
        params: { id: this.$route.params.id, startTime: this.$route.params.startTime, endTime: this.$route.params.endTime } })
      // ReportMarketDeptDetail
