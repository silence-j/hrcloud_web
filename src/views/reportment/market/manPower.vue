<template>
  <div class="container"  @click="mutilTreeData.showTree=false">
    ...
    <div class="mainForm mt-20">
      ...
      <el-row>
        <el-col class="colTitleWidth">其中达到法定退休年龄</el-col>
...
<script>
import { getMarketManPower, getMarketDatUser } from 'static/api/report/index'
...
export default {
  ...
  methods: {
    selectChange (orgId) {
      this.manForm.orgIds = orgId
    },
    _getManDeatil () {
      if (this.manForm.orgIds.length) {
        getMarketManPower(this.manPowerParams).then(res => {
          if (res && res.result) {
    ...
    _getDataUser () {
      if (this.manForm.orgIds.length) {
        let dataUserParams = {
          orgIds: this.manPowerParams.orgIds,
          type: this.dataUserType
        }
        getMarketDatUser(dataUserParams).then(res => {
          if (res && res.result) {
  ...
  computed: {
    manPowerParams () {
      return {
        orgIds: this.mutilTreeData.orgIds ? this.mutilTreeData.reportOrgIds : [getToken('loginDeptId')]
