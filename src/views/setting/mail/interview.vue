...
<div class="interview-box">
  <el-table :data="interviewArr" border class="cell-h46">
    <el-table-column
      prop="no"
      label="模板编号">
    </el-table-column>
    <el-table-column
      ...
      <template slot-scope="scope">
        <div class="line-clamp2" :title="scope.row.name">{{scope.row.name}}</div>
      </template>
    </el-table-column>
    <el-table-column
      prop="description"
...
export default {
  ...
  methods: {
    ...
    // 编辑邮件模板
    modifyMail (row) {
      this.$router.push({ name: 'InterviewEdit', params: { id: row.id } })
    },
    // 如果没有面试模板的权限则跳转offer模板页面
    jugeInterviewOrOffer () {
      if (this.permission.indexOf('面试模板管理') === -1) {
