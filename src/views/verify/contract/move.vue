<template>
  <div class="container">
    ...
    @tab-click="tabClick">
      ...
      <el-tab-pane v-if="getCurrentRoleGroups.indexOf('contractRelease')!==-1">
  ...
  methods: {
    ...
    UnagreeSave () {
      /* if (this.UnagreeForm.backreson === '') {
          ...
          this.unComplateForm.var.values = false + ',' + this.UnagreeForm.backreson
        } */
    ...
    /** 待审核**/
    _getwaitTaskList () {
      getTaskList(Object.assign(this.contractWaitParams, { groupId: 'contractRelease' })).then(data => {
        if (data.result) {
          if (data.result.records) {
