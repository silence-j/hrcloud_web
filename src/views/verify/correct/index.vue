     ...
     <el-form :model="correctForm">
       <div class="search-form_box clearfix">
         <div class="search-form_item">
           <el-input v-model="correctForm.namePhone" placeholder="请输入姓名/身份证号" @keyup.enter.native="searchContractList"></el-input>
         </div>
      ...
      <div class="tab-wrap clearfix">
        <ul class="edit-tab clearfix">
          <li @click="emptyClearMsg">
             <router-link :to="'/verify/correct/started'">我发起的</router-link>
          </li>
          <li v-if="getCurrentRoleGroups.indexOf('correctHr') !== -1" @click="emptyClearMsg">
            <router-link :to="'/verify/correct/personalAudit'">人事负责人审核</router-link>
          </li>
          <li v-if="getCurrentRoleGroups.indexOf('headHr') !== -1" @click="emptyClearMsg">
            <router-link :to="'/verify/correct/groupAudit'">集团总部审核</router-link>
          </li>
        </ul>
      </div>
    </el-col>
...
export default {
  ...
  computed: {
    ...
    pickerOptions () {
      return {
        disabledDate: (time) => {
          return new Date(time).getTime() < new Date(this.correctForm.startTime).getTime()
        }
      }
    },
    permission () {
  ...
  methods: {
    setSerchList (boolean) {
      this.$store.dispatch('setCorrectSearch', this.correctForm)
      this.$store.commit('SET_CORRECT_CLICKED', boolean)
    },
    searchContractList () {
