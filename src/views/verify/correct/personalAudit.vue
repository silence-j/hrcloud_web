...
export default {
  ...
  methods: {
    ...
    /** 待审核**/
    _getWaitTaskList () {
      getTaskList(Object.assign(this.contractWaitParams)).then(data => {
        if (data.result) {
    ...
    // "var":{"keys":"correctHrPass","values":"true","types":"S"}
    // "var":{"keys":"correctHrPass,correctHrBackReason","values":"false,拒绝原因","types":"S,S"}
    // 不同意
    resonComfire () {
      // console.log('5:', this.$refs.reasonValidateForm.validate)
      this.$refs.reasonForm.validate(valid => {
        console.log(valid, '789789', this.reasonForm.reason)
        if (valid) {
          let row = this.row
          let params = {
            actType: 'correct',
