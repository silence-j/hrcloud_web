<template>
  <div>
    <!--普通管理人员权限看到的页面-->
    <GrantMg :type="'实物'" v-if="permission.indexOf('上报者')==-1 && permission.indexOf('管理者')!==-1"></GrantMg>
    <!--管理者权限进去看到的页面-->
    <ReportMg :type="'实物'" v-if="permission.indexOf('上报者')!==-1 && permission.indexOf('管理者')==-1"></ReportMg>
    ...
    <lenTabs :indexLiProp="indexLi" :activeTabIndexProp="activeTabIndex" v-if="permission.indexOf('上报者')!=-1 && permission.indexOf('管理者')!=-1" :tabDatas="msgRemind" @getItem="getWhichPage">
      <div slot="tableBox1">
        <GrantMg :type="'实物'"></GrantMg>
      </div>
      <div slot="tableBox2">
        <ReportMg :type="'实物'"></ReportMg>
      </div>
    </lenTabs>
  </div>
</template>
