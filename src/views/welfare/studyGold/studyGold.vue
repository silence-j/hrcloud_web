<template>
  <div class="salary">
    ...
    <el-button @click="exportList" v-show="permission.indexOf('导出')!=-1" type="primary" size="small">导出</el-button>
    <div class="mt-20">
    ...
    <div class="search-data">
      <el-table :data="studyList">
        ...
        <el-table-column prop="entryDate" label="入司时间" width="145px">
          <template slot-scope="scope">
            <span class="brokenStyle">{{scope.row.entryDate}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="duty" label="职务" width="145px">
          <template slot-scope="scope">
            <span class="brokenStyle">{{scope.row.duty}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="childrenName" label="子女姓名" width="145px">
          <template slot-scope="scope">
            <span class="brokenStyle">{{scope.row.childrenName}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="relation" label="与申请人关系" width="225px">
          <template slot-scope="scope">
            <span class="brokenStyle">{{scope.row.relation}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="childrenIdCard" label="子女身份证号" width="125px"></el-table-column>
        <el-table-column prop="school" label="学校名称" width="125px"></el-table-column>
        <el-table-column prop="eductionalSystem" label="学制" width="135px"></el-table-column>
        <el-table-column prop="startSchoolTime" label="入学时间" width="135px"></el-table-column>
        <el-table-column prop="validYear" label="可申领年限" width="135px"></el-table-column>
        <el-table-column prop="usedYear" label="已申领年限" width="135px"></el-table-column>
        <el-table-column prop="companyName" label="所在公司" width="135px"></el-table-column>
        <el-table-column prop="post" label="岗位" width="135px"></el-table-column>
        <el-table-column prop="currentYear" label="申领日期" width="135px"></el-table-column>
        <el-table-column prop="sales" label="销量" width="135px"></el-table-column>
    ...
    <form  style="display:none" name="form" method="POST" ref="submitExcel" target="_blank" :action="hostBaseUrl+'/duty/studentGrant/export'">
...
export default {
  ...
  computed: {
    ...mapGetters(['getOrgId']),
    permission () {
      return getExmine().welfareStudyGold
    },
  ...
  methods: {
    ...
    _getStudyList () {
      ...
      WelfareApi.getStudyList(params).then(res => {
        if (res.result.records) {
          this.studyList = res.result.records
          console.log(this.studyList, '555')
